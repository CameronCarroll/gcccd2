...
\newcommand{\hmwkTitle}{HW Set 12/13} % Assignment title
\newcommand{\hmwkDueDate}{Monday,\ November\ 27,\ 2013} % Due date
...
\begin{homeworkProblem}[Problem 10.1] % Custom section title
  Consider this sugar-water phase diagram, figure 10-1; How much sugar will dissolve in 1 kg of water at $80\,^{\circ}\mathrm{C}$? \\[0.2cm]
  If the saturated liquid solution is cooled to $20\,^{\circ}\mathrm{C}$, some sugar will precipitate out as a solid. What will the composition of the saturated liquid solution be at that temperature? \\[0.2cm]
  How much of the solid sugar will come out of solution upon cooling to $20\,^{\circ}\mathrm{C}$? \\[0.2cm]
  \begin{figure}*
    \caption{FundMatSciEng, 3rd Edition, Figure 10-1}
    \centerline{\includegraphics[width=13cm]{fig10-1}}
  \end{figure}
  \problemAnswer{
    \begin{enumerate}
      \item Well, moving horizontally to the solubility line from the 80 degree point and then down vertically to the composition, we find roughly 75\% sugar and 25\% water. \\[0.25cm]
      $C_{sugar} = \frac{m_{sugar}}{m_{sugar} + m_{water}} (100 wt\%)$ \\[0.25cm]
      $0.75 = \frac{m_{sugar}}{m_{sugar} + m_{water}}$ \\[0.25cm]
      $m_{sugar} = 3 \cdot m_{water}$ \\[0.25cm]
      $m_{sugar} = 3\ kg$
      \item It looks to be roughly 63wt\% sugar. \\[0.2cm]
      $0.63(m_s+m_w) = m_s$ \\[0.2cm]
      $.63(m_w) = m_s(1-.63)$ \\[0.2cm]
      $m_s = \frac{.63}{1-.63}(m_w) = 1.7(m_w)$ \\[0.2cm]
      So the new solution only holds 1.7 kg of sugar and 1.3 kg must come out of solution.
    \end{enumerate}
  }
\end{homeworkProblem}
\clearpage
...
\begin{homeworkProblem}[Example 10.2] % Custom section title
